...
from gdt.gts import match_gts_obs_to_dac_profiles


def main(args):
    """Match released GTS observations to available IOOS Glider DAC profiles for the specified dataset ID.
    ...
    logging.debug('Creating OSMC ERDDAP connection: {:}'.format(end_points['OSMC_ERDDAP_URL'].url))
    ...
    osmc_profiles = osmc_client.get_dataset_profiles(client.datasets, gps=True)
    ...
    aligned_profiles = match_gts_obs_to_dac_profiles(osmc_profiles, dac_profiles)
    if not aligned_profiles.empty:
        rename_columns = {'profile_id': 'dac_profile_id',
                          'latitude': 'dac_latitude',
                          'longitude': 'dac_longitude'}
        aligned_profiles = aligned_profiles.rename(columns=rename_columns)
#        aligned_profiles.dac_profile_id = aligned_profiles.dac_profile_id.astype(int)
        aligned_profiles['dataset_id'] = dataset_id
