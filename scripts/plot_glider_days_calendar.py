...
def main(args):
    """
    Search the IOOS Glider DAC for data sets matching the search criteria and plot the glider days calendar. Calendar is
    ...
    if plot_type == 'ymd':
        ...
        logging.info('Plotting year,month,day glider days calendar')
        ...
        calendar = client.ymd_glider_days_calendar
        ...
        logging.info('{:} total glider days in the specified time window'.format(calendar.sum().sum()))
        ax.set_title('{:} Glider Days ({:} Deployments): {:} - {:}'.format(title,
                                                               datasets.shape[0],
        ...
        logging.info('Plotting year,month glider days calendar')
